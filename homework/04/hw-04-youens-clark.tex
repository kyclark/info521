...
\\
(1 - r)(\delta - 1) &= -r(\gamma - 1)
\\
\delta - 1 - r \delta + r &= r - r \gamma
\\
r \gamma - r \delta &= 1 - \delta
\\
r (\gamma - \delta) &= 1 - \delta
\\
\hat{r} &= \frac{1 - \delta}{\gamma - \delta} 
\\
\hat{r} &= \frac{1 - (y_N + \alpha)}{(N - y_N + \beta) - (y_N + \alpha)}
\\
\hat{r} &= \frac{1 - y_N - \alpha}{N - 2 y_N - \alpha + \beta}
\\
\frac{\partial \log(f)}{\partial r} &= r^{-1}(\delta - 1) + (1 - r)^{-1}(\gamma - 1) = 0
\\
\frac{\partial^2 \log(f)}{\partial r} 
&= r^{-2}(1 - \delta) - (r - 1)^{-2}(\gamma - 1)
\\
&= \frac{1 - (y_N + \alpha)}{r^2} - \frac{(N - y_N + \beta - 1)}{(r - 1)^2}
\end{aligned}
\end{eqnarray*}
