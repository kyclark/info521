...
def get_args():
    ...
    parser.add_argument('-m', '--max_order',
...
def run_cv(K, maxorder, x, t, testx, testt,
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            ### YOUR CODE HERE
            w = np.linalg.inv((trainX.transpose().dot(trainX))).dot(trainX.transpose().dot(traint))
    ...
    plot_cv_results(log_mean_train_loss, log_mean_cv_loss, log_mean_ind_loss,
                    log_scale_p=True, plot_title=title, filepath=outfile)
...
def main():
    ...
    for n in range(args.num_runs):
        #
        # Read data from file, separate into x/t
        #
        data = permute_rows(np.genfromtxt(filename, delimiter=',', dtype=None))
        x = data[:, 0] # all rows, 1st column
        t = data[:, 1] # all rows, 2nd column
        N = x.size

        if K > N:
            die('K "{}" must be < number of rows "{}"'.format(K, N))

        # Storage for the design matrix used during training
        # Here we create the design matrix to hold the maximum sized
        # polynomial order
        # When computing smaller model polynomial orders below, we'll just
        # use the first 'k+1' columns for the k-th order polynomial.  This
        # way we don't have to keep creating new design matrix arrays.
        X = np.zeros((N, maxorder + 1))
    ...
    #    testX = np.zeros((testx.shape[0], maxorder + 1))
        ...
        #print('folds = {}'.format(fold_indices))
        ...
        for p in range(maxorder + 1):
            print('p = {}'.format(p))
            # Augment the input data by the polynomial model order
            # E.g., 2nd-order polynomial model takes input x to the 0th,
            # 1st, and 2nd power
            X[:, p] = np.power(x, p)
            print(X)
            ...
            for fold in range(K):
                ...
                trainX = np.copy(X[:, 0:p + 1])
                # remove the fold x from the training set
                trainX = np.delete(trainX,
                                   np.arange(fold_indices[fold],
                                             fold_indices[fold + 1]), 0)
                ...
                # remove the fold t from the training set
                traint = np.delete(traint,
                                   np.arange(fold_indices[fold],
                                             fold_indices[fold + 1]), 0)

                print('foldX = {}'.format(foldX))
                print('foldt = {}'.format(foldt))
                print('trainX = {}'.format(trainX))
                print('traint = {}'.format(traint))
        ...
        # Plot log scale loss results
        plot_cv_results(log_mean_train_loss,
                        log_mean_cv_loss,
                        None,
                        log_scale_p=True,
                        plot_title=title,
