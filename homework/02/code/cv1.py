...
def run_cv(K, maxorder, x, t, testx, testt, randomize_data=False, title='CV'):
    ...
    # Storage for the design matrix used during training Here we create the
    # design matrix to hold the maximum sized polynomial order When computing
    # smaller model polynomial orders below, we'll just use the first 'k+1'
    # columns for the k-th order polynomial.  This way we don't have to keep
    # creating new design matrix arrays.
    ...
    fold_indices = list(map(lambda x: int(x), np.linspace(0, N, K + 1)))
    ...
    for p in range(maxorder + 1):
        ...
        # E.g., 2nd-order polynomial model takes input x to the 0th, 1st, and
        # 2nd power
        X[:, p] = np.power(x, p)
        ...
        for fold in range(K):
            ...
            # safely copy the training data (so that deleting doesn't remove
            # the original
            traint = np.copy(t)
            ...
            #print(X)
            #print(t)
            #w, res, rank, s = np.linalg.lstsq(trainX, traint)
            ...
            ind_pred = np.dot(testX[:, 0:p + 1],
                              w)  # model predictions on independent test data
...
def run_demo():
    ...
    #    testx, testt, = generate_synthetic_data(
    #        1000, w, xmin=xmin, xmax=xmax, sigma=sigma)
    #    print('x = {}'.format(x))
    #    print('t = {}'.format(t))
    #    print('testx = {}'.format(testx))
    #    print('testt = {}'.format(testt))
    #    plot_data_and_model(x, t, w)

    data = np.genfromtxt('synthdata2018.csv', delimiter=',', dtype=None)
    x = data[:20, 0]
    t = data[:20, 1]
    testx = data[20:, 0]
    testt = data[20:, 1]
    ...
    #testt = data[n:, 1]
    #random.shuffle(data)
    #training_size = int(d.size * .8)
    #train_on = random.sample(range(d.size), int(d.size * .8))
    #x = data[train_on]
    #t = np.delete(data, train_on)
    #print(data)
    #exit()
    #x = data[:training_size, 0]
    #t = data[:training_size, 1]
    #    print(x)
    #    print(t)
    #testx = data[training_size:, 0]
    #testt = data[training_size:, 1]

    K = 10
    print('x = {}'.format(len(x)))
    print('testx = {}'.format(len(testx)))
